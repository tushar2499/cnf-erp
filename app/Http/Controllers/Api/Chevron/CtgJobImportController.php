...
/**
 * Chittagong (branch "CGP") job importer.
 *
 * Reads the "JOB INFO" sheet of the Chittagong workbook. Must be run before
 * CTG job-expense import so expenses link up to existing jobs.
 */
class CtgJobImportController extends Controller
{
    private const CTG_BRANCH_ID = 6;
    ...
    private function loadRows(string $pathname): array
    {
        ...
        $sheet = $spreadsheet->getSheetByName('JOB INFO') ?? $spreadsheet->getActiveSheet();
    ...
    /**
     * CTG job sheet columns (A–AA): Sl, Action, Job No, Job Date, Job Type,
            ...
            foreach ($rows as $row) {
                ...
                if ($portId === null && $rawPortName !== '') {
                    $port = ChevronPort::firstOrCreate(
                        ...
                        ['branch_id' => self::CTG_BRANCH_ID, 'is_active' => true]
                    );
                    $portId = $port->id;
                }
