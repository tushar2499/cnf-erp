...
class BranchController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            return DataTables::of(NasTradingBranch::query())
                ->addIndexColumn()
                ...
                ->addColumn('action', fn($r) =>
                    ...
                    <button class="btn btn-sm btn-outline-danger btn-delete"
                        data-url="' . route('nas-trading.settings.branches.destroy', $r->id) . '"
                        data-name="' . e($r->name) . '">
        ...
        return view('nas-trading.settings.branches.index');
    }

    public function store(Request $request)
    {
        $request->validate(['name' => 'required|string|max:255']);
        NasTradingBranch::create([
            'name'      => $request->name,
    ...
    public function update(Request $request, NasTradingBranch $branch)
    {
        $request->validate(['name' => 'required|string|max:255']);
        $branch->update([
            'name'      => $request->name,
    ...
    public function destroy(NasTradingBranch $branch)
    {
        $branch->delete();
