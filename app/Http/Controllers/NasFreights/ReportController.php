...
class ReportController extends Controller
{
    ...
    public function bookingReportExcel(BookingReportRequest $request)
    {
        ...
        if ($request->filled('from_date')) {
            $dateStr .= 'From: '.Carbon::parse($request->from_date)->format('d/m/Y').'   ';
        }
        ...
        $headers = ['SL', 'Job No', 'Job Date', 'Entry By', 'Sales Person', 'Customer', 'Supplier', 'Cover Van Details', 'Challan No', 'Location', 'Supplier Rate', 'Customer Rate', 'Profit', 'Remarks', 'Billed', 'Bill No'];
        $col = 'A';
        ...
        foreach ($rows as $i => $item) {
            ...
            $tp += $profit;
            $sheet->fromArray([$i + 1, $b?->job_no, $b?->job_date?->format('d M Y'), $b?->entry_by, $b?->sales_person_name, $b?->customer_name, $item->supplier_name, $item->cover_van_no, $item->challan_no ?? '', $loc, (float) $item->supplier_rate, (float) $item->customer_rate, (float) $profit, $b?->note, $item->is_billed ? 'Billed' : 'Pending', $item->bill_no ?? ''], null, 'A'.$row);
            foreach (['K', 'L', 'M'] as $c) {
        ...
        foreach (['A' => 4, 'B' => 14, 'C' => 12, 'D' => 10, 'E' => 14, 'F' => 20, 'G' => 20, 'H' => 16, 'I' => 14, 'J' => 28, 'K' => 14, 'L' => 14, 'M' => 14, 'N' => 24, 'O' => 10, 'P' => 16] as $c => $w) {
    ...
    public function partyBillSummaryExcel(PartyBillSummaryRequest $request)
    {
        ...
        $sheet->setCellValue('A2', 'Transport Bill Summary');
        $sheet->getStyle('A2')->applyFromArray(['font' => ['bold' => true, 'size' => 12], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]);
        ...
        if ($customer) {
            ...
            $sheet->setCellValue('A4', 'Customer: '.$customer->name);
        ...
        foreach ($bills as $i => $bill) {
            ...
            $tNet += $bill->sub_total;
            $tTds += $bill->tds_amount;
            $tVat += $bill->vat_amount;
            $tTotal += $bill->total_amount;
        ...
        $sheet->setCellValue('A'.$row, 'Total ('.$bills->count().' bills)');
        $sheet->setCellValue('G'.$row, $tNet);
        $sheet->setCellValue('I'.$row, $tTds);
        $sheet->setCellValue('K'.$row, $tVat);
        $sheet->setCellValue('L'.$row, $tTotal);
        ...
        foreach (['A' => 4, 'B' => 16, 'C' => 18, 'D' => 12, 'E' => 20, 'F' => 20, 'G' => 14, 'H' => 8, 'I' => 12, 'J' => 8, 'K' => 12, 'L' => 14, 'M' => 20] as $c => $w) {
            $sheet->getColumnDimension($c)->setWidth($w);
        }
        if ($row > $headerRow + 1) {
            $sheet->getStyle('A'.$headerRow.':M'.$row)->applyFromArray(['borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'CCCCCC']]]]);
        }
    ...
    public function billDetailsExcel(BillDetailsRequest $request)
    {
        ...
        $sheet->mergeCells('A2:M2');
        $sheet->setCellValue('A2', 'Monthwise Transport Bill Details');
        $sheet->getStyle('A2')->applyFromArray(['font' => ['bold' => true, 'size' => 12], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]);
        ...
        $sheet->setCellValue('A'.$row, 'Total ('.$items->count().' items)');
        $sheet->setCellValue('I'.$row, $tNet);
        $sheet->setCellValue('J'.$row, $tDem);
        $sheet->setCellValue('L'.$row, $tVat);
        $sheet->setCellValue('M'.$row, $tTotal);
        $sheet->getStyle('A'.$row.':M'.$row)->applyFromArray(['font' => ['bold' => true], 'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'D4E8D4']]]);
        $sheet->getStyle('A'.$row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        foreach (['I', 'J', 'L', 'M'] as $c) {
            $sheet->getStyle($c.$row)->getNumberFormat()->setFormatCode('#,##0.00');
        ...
        foreach (['A' => 4, 'B' => 18, 'C' => 12, 'D' => 14, 'E' => 16, 'F' => 10, 'G' => 18, 'H' => 18, 'I' => 14, 'J' => 12, 'K' => 8, 'L' => 12, 'M' => 14] as $c => $w) {
            $sheet->getColumnDimension($c)->setWidth($w);
        }
        if ($row > $headerRow + 1) {
            $sheet->getStyle('A'.$headerRow.':M'.$row)->applyFromArray(['borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'CCCCCC']]]]);
    ...
    private function getBookingRows(Request $request): Collection
    {
        ...
        $rows = NasFreightsBookingItem::with('booking')
            ->whereHas('booking', function ($q) use ($request) {
                ...
                if ($request->filled('from_date')) {
                    $q->whereDate('job_date', '>=', $request->from_date);
                }
                if ($request->filled('to_date')) {
                    $q->whereDate('job_date', '<=', $request->to_date);
                }
    ...
    private function getBillItems(Request $request): array
    {
        ...
        $bookingItems = NasFreightsBookingItem::whereIn('booking_id', $bookingIds)
            ->get()
            ->groupBy('booking_id');
