...
@section('title', $expense ? 'Edit Expense — '.$expense->expense_no : ($bookingType === 'export' ? 'Export Expense Entry' : 'Import Expense Entry'))
...
<form id="expenseForm" method="POST"
      action="{{ $expense ? route($routePrefix.'.update', $expense->id) : route($routePrefix.'.store') }}">
    @csrf
                    ...
                    <div class="row g-2">
                        <div class="col-12">
                            <label class="form-label">
                                {{ $bookingType === 'export' ? 'Export Booking No' : 'Import Booking No' }}
                            </label>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" id="totalExpDisplay" class="form-control form-control-sm ro-field" readonly
                ...
                <div class="col-md-6">
                    <div class="row g-2">
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" id="totalAppDisplay" class="form-control form-control-sm ro-field" readonly
                            ...
                            <td>
                                <select name="rows[{{ $i }}][expense_head_id]" class="form-select form-select-sm expense-head-select">
                                    ...
                                    @foreach($expenseHeads as $head)
                                        <option value="{{ $head->id }}" data-amount="{{ $head->amount ?? '' }}"
                                            {{ $row?->expense_head_id == $head->id ? 'selected' : '' }}>{{ $head->name }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td>
...
@push('scripts')
<script>
var SEARCH_BOOKINGS = '{{ route($routePrefix.'.search-bookings') }}';
var TODAY           = '{{ $today }}';
...
$(function () {
    ...
    $('#bookingSelect').select2({
        ...
        placeholder: 'Enter minimum 1 character',
        allowClear: true, minimumInputLength: 1,
        ajax: {
            url: SEARCH_BOOKINGS, dataType: 'json', delay: 250,
            data: p => ({ q: p.term }),
