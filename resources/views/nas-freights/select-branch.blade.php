...
@section('title', 'Select Branch — NAS Freights')
...
<div style="width:100%; max-width:680px;" class="mx-3">
    <div class="text-center mb-4">
        ...
        <p style="color:#c8d0e0;" class="small">
            <strong>NAS Freights</strong> — Select your branch to continue.
        </p>
    </div>
    ...
    <div class="row g-3 justify-content-center">
        ...
        <div class="col-12 col-sm-6 col-md-4">
            <form method="POST" action="{{ route('nas-freights.select-branch.store') }}">
                @csrf
                ...
                <button type="submit" class="card border-0 shadow w-100 text-start p-0"
                    ...
                    <div class="card-body p-4 text-center">
                        <div class="mb-3" style="width:54px;height:54px;border-radius:14px;background:rgba(255,193,7,.12);display:flex;align-items:center;justify-content:center;margin:0 auto;font-size:1.4rem;color:#ffc107;">
                            <i class="fa fa-code-branch"></i>
                        </div>
                        <h6 class="fw-bold mb-1" style="font-size:.9rem;color:#1e293b;">{{ $branch->name }}</h6>
                        @if($branch->code)
                        <span class="badge bg-warning bg-opacity-10 text-warning" style="font-size:.68rem;letter-spacing:.05em;">{{ $branch->code }}</span>
                        @endif
                        @if($branch->address)
                        <p class="text-muted mt-2 mb-0" style="font-size:.72rem;">{{ $branch->address }}</p>
                        @endif
                        <div class="mt-3">
                            <span class="btn btn-sm btn-outline-warning w-100">Select <i class="fa fa-arrow-right ms-1"></i></span>
                        </div>
                    </div>
                </button>
            </form>
