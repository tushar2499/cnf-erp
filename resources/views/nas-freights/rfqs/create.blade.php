...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-2">
        ...
        <div class="fw-bold" style="font-size:.9rem; color:#0a4f3c;">
            RFQ Entry @if ($rfq)
                <span class="ms-2 badge bg-light text-dark border">{{ $rfq->rfq_no }}</span>
            @endif
        </div>
        <div>
            ...
            <div class="section-body">
                <div class="row g-2 mb-2">
                    <div class="col-md-2">
                        ...
                        <input type="text" class="form-control rfq-input bg-light"
                            value="{{ $rfq?->rfq_no ?? 'Auto Generated' }}" readonly>
                    </div>
                    <div class="col-md-2">
                        <div class="rfq-label">RFQ Date <span class="text-danger">*</span></div>
                        <input type="date" name="rfq_date" class="form-control rfq-input"
                            value="{{ old('rfq_date', $rfq?->rfq_date?->format('Y-m-d') ?? $today) }}" required>
                    </div>
                    <div class="col-md-2">
                        <div class="rfq-label">Valid Until</div>
                        <input type="date" name="valid_until" class="form-control rfq-input"
                            value="{{ old('valid_until', $rfq?->valid_until?->format('Y-m-d') ?? $defaultValidUntil) }}">
                    </div>
                    <div class="col-md-3">
                        <div class="rfq-label">Customer</div>
                        <select name="customer_id" id="customerSelect" class="form-select rfq-input" style="width:100%">
                            @if ($rfq?->customer_id)
                                <option value="{{ $rfq->customer_id }}" selected>{{ $rfq->customer?->customer_id }} —
                                    {{ $rfq->customer?->name }}</option>
                            @endif
                        </select>
                    </div>
                    <div class="col-md-3">
                        <div class="rfq-label">Salesperson</div>
                        <select name="salesperson_id" id="salespersonSelect" class="form-select rfq-input"
                            style="width:100%">
                            @if ($rfq?->salesperson_id)
                                <option value="{{ $rfq->salesperson_id }}" selected>{{ $rfq->salesperson?->name }}
                                </option>
                            @endif
                        </select>
                    </div>
                ...
                <div class="row g-2 mb-2">
                    <div class="col-md-6">
                        ...
                        <select name="overseas_agent_id" id="overseasAgentSelect" class="form-select rfq-input"
                            style="width:100%">
                            @if ($rfq?->overseas_agent_id)
                                <option value="{{ $rfq->overseas_agent_id }}" selected>
                                    {{ $rfq->overseasAgent?->agent_code }} — {{ $rfq->overseasAgent?->name }}
                                    ({{ $rfq->overseasAgent?->country }})
                                </option>
                            @endif
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="shipping_carrier_id" id="shippingCarrierSelect" class="form-select rfq-input"
                            style="width:100%">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="type" class="form-select rfq-input" required>
                            @foreach ($types as $val => $label)
                                <option value="{{ $val }}"
                                    {{ old('type', $rfq?->type ?? 'import') === $val ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="service_type" class="form-select rfq-input">
                            @foreach ($serviceTypes as $st)
                                <option value="{{ $st }}"
                                    {{ old('service_type', $rfq?->service_type ?? 'FCL') === $st ? 'selected' : '' }}>
                                    {{ $st }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="incoterms" class="form-select rfq-input">
                            ...
                            @foreach ($incoterms as $inc)
                                <option value="{{ $inc }}"
                                    {{ old('incoterms', $rfq?->incoterms) === $inc ? 'selected' : '' }}>
                                    {{ $inc }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="currency" class="form-select rfq-input">
                            @foreach ($currencies as $cur)
                                <option value="{{ $cur }}"
                                    {{ old('currency', $rfq?->currency ?? 'BDT') === $cur ? 'selected' : '' }}>
                                    {{ $cur }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="row g-2 mb-2">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="pol" class="form-control rfq-input"
                            value="{{ old('pol', $rfq?->pol) }}" placeholder="e.g. Chittagong">
                    </div>
                    <div class="col-md-3">
                        <div class="rfq-label">Port of Discharge (POD)</div>
                        <input type="text" name="pod" class="form-control rfq-input"
                            value="{{ old('pod', $rfq?->pod) }}" placeholder="e.g. Singapore">
                    </div>
                    <div class="col-md-3">
                        <div class="rfq-label">Place of Receipt</div>
                        <input type="text" name="place_of_receipt" class="form-control rfq-input"
                            value="{{ old('place_of_receipt', $rfq?->place_of_receipt) }}"
                            placeholder="Pre-carriage origin">
                    </div>
                    <div class="col-md-3">
                        <div class="rfq-label">Place of Delivery</div>
                        <input type="text" name="place_of_delivery" class="form-control rfq-input"
                            value="{{ old('place_of_delivery', $rfq?->place_of_delivery) }}"
                ...
                <div class="row g-2">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="commodity_description" class="form-control rfq-input"
                            value="{{ old('commodity_description', $rfq?->commodity_description) }}"
                            placeholder="General cargo description">
                    </div>
                    <div class="col-md-6">
                        <div class="rfq-label">Remarks</div>
                        <input type="text" name="remarks" class="form-control rfq-input"
                            value="{{ old('remarks', $rfq?->remarks) }}" placeholder="Special instructions or notes">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="section-card">
                ...
                <div class="section-body">
                    <div class="row g-3 align-items-start">
                                ...
                                @foreach (['Draft' => ['secondary', 'Draft'], 'Pending' => ['warning', 'Pending — Sent to Client'], 'Win' => ['success', 'Win'], 'Lose' => ['danger', 'Lose']] as $val => [$color, $label])
                                    <div class="form-check ps-0">
                                        ...
                                        <label class="form-check-label" for="edit_status_{{ $val }}">
                                            <span
                                                class="badge bg-{{ $color }} {{ $color === 'warning' ? 'text-dark' : '' }}">{{ $val }}</span>
                        ...
                        <div class="col-md-3" id="editLostReasonWrap"
                            style="{{ old('status', $rfq->status) === 'Lose' ? '' : 'display:none' }};">
                            ...
                            <select name="lost_reason" class="form-select form-select-sm">
                                ...
                                @foreach ($lostReasons as $reason)
                                    <option value="{{ $reason }}"
                                        {{ old('lost_reason', $rfq->lost_reason) === $reason ? 'selected' : '' }}>
                                        {{ $reason }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    @if ($rfq && $rfq->status === 'Win' && $rfq->type === 'import' && !$rfq->converted_freight_booking_id)
        <form id="convertBookingForm" method="POST"
            action="{{ route('nas-freights.rfqs.convert-freight-booking', $rfq->id) }}">
            @csrf
        </form>
    @endif
    ...
    <template id="itemTemplate">
        <tr>
            ...
            <td>
                <input type="text" name="items[0][hs_code]" class="form-control form-control-sm"
                    style="font-size:.72rem;" placeholder="e.g. 4203.21">
            </td>
            <td>
                <input type="text" name="items[0][commodity]" class="form-control form-control-sm"
                    style="font-size:.72rem;" placeholder="e.g. Leather jackets">
            </td>
            <td>
                <input type="number" name="items[0][gross_weight]" class="form-control form-control-sm text-end"
                    style="font-size:.72rem;" step="0.01" placeholder="0.00">
            </td>
            <td>
            ...
            <td>
                <input type="number" name="items[0][volume_cbm]" class="form-control form-control-sm text-end"
                    style="font-size:.72rem;" step="0.001" placeholder="0.000">
            </td>
            <td>
                <input type="text" name="items[0][country_of_origin]" class="form-control form-control-sm"
                    style="font-size:.72rem;" placeholder="e.g. Bangladesh">
            </td>
            <td>
            ...
            <td>
                <input type="text" name="items[0][special_handling]" class="form-control form-control-sm"
                    style="font-size:.72rem;" placeholder="e.g. Fragile">
            </td>
        </tr>
    </template>
@endsection
