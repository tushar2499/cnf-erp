...
@push('styles')
    <style>
        #containerTypesTable th,
        #containerTypesTable td {
            white-space: nowrap;
            font-size: .73rem;
            padding: .3rem .5rem;
        }

        #containerTypesTable thead tr:first-child th {
            background: #e9ecef;
            font-weight: 600;
            position: sticky;
            top: 0;
        ...
        #containerTypesTable thead tr:last-child th {
            background: #f8f9fa;
            font-weight: normal;
            position: sticky;
            z-index: 2;
        ...
        #containerTypesTable thead tr:last-child th input.form-control {
            min-width: 120px;
            width: 100%;
        ...
        .container-types-table-wrapper {
            max-height: 65vh;
            overflow: auto;
        }

        .container-types-table-wrapper::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .container-types-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .container-types-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #containerTypesTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <span><i class="fa fa-list me-2"></i> All Container Types</span>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item"
                        onclick="$('#containerTypesTable').DataTable().button('.buttons-csv').trigger()"><i
                            class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item"
                        onclick="$('#containerTypesTable').DataTable().button('.buttons-excel').trigger()"><i
                            class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item"
                        onclick="$('#containerTypesTable').DataTable().button('.buttons-pdf').trigger()"><i
                            class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item"
                        onclick="$('#containerTypesTable').DataTable().button('.buttons-print').trigger()"><i
                            class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="card-body p-0">
        <div class="container-types-table-wrapper">
            <table id="containerTypesTable" class="table table-hover table-striped table-bordered mb-0">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Name</th>
...
$(function () {
    table = $('#containerTypesTable').DataTable({
        ...
        initComplete: function () {
            const firstRowH = $('#containerTypesTable thead tr:first-child').outerHeight();
            $('#containerTypesTable thead tr:last-child th').css('top', firstRowH + 'px');
