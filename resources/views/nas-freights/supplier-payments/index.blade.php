...
@push('styles')
    <style>
        ...
        #payTable thead tr:first-child th {
            background: #1a6b60;
            color: #fff;
            font-weight: 600;
            position: sticky;
        ...
        #payTable thead tr:last-child th {
            background: #f8f9fa;
            font-weight: normal;
            position: sticky;
            z-index: 2;
        ...
        #payTable thead tr:last-child th input.form-control {
            min-width: 72px;
            width: 100%;
        ...
        .pay-table-wrapper::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .pay-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .pay-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #payTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><button class="dropdown-item"
                                onclick="$('#payTable').DataTable().button('.buttons-csv').trigger()"><i
                                    class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                        <li><button class="dropdown-item"
                                onclick="$('#payTable').DataTable().button('.buttons-excel').trigger()"><i
                                    class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                        <li><button class="dropdown-item"
                                onclick="$('#payTable').DataTable().button('.buttons-pdf').trigger()"><i
                                    class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>
                        <li><button class="dropdown-item"
                                onclick="$('#payTable').DataTable().button('.buttons-print').trigger()"><i
    ...
    <script>
        $(function() {
            var table = $('#payTable').DataTable({
                ...
                ajax: {
                    url: '{{ route('nas-freights.supplier-payments.index') }}',
                    data: function(d) {
                ...
                language: {
                    emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-hand-holding-usd fa-2x mb-2 d-block"></i>No payments yet.</div>'
                },
                initComplete: function() {
                    const firstRowH = $('#payTable thead tr:first-child').outerHeight();
                    $('#payTable thead tr:last-child th').css('top', firstRowH + 'px');
