...
@section('title', 'New Customer Bill')
...
<div class="page-header">
    <h4><i class="fa fa-file-invoice-dollar me-2 text-info"></i> New Customer Bill</h4>
    ...
    <div class="p-3">
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md-5">
                ...
                <input type="hidden" id="fldCustomerId" name="customer_id">
                ...
                <div class="col-md-8">
                    <div class="row g-2">
                        ...
                        <div class="col-12">
                            ...
                            <textarea id="fldBillAddress" name="customer_address" class="form-control form-control-sm" rows="2"></textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label">Note</label>
                            <textarea id="fldNote" name="note" class="form-control form-control-sm" rows="2"></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <table style="width:100%;border-collapse:collapse">
                        <tr>
                            ...
                            <td class="py-1">
                                <div class="input-group input-group-sm">
                                    <input type="number" id="fldTdsPct" name="tds_percent" class="form-control form-control-sm" value="0" min="0" step="0.01" oninput="recalcBill()">
                        ...
                        <tr>
                            ...
                            <td class="py-1">
                                <div class="input-group input-group-sm">
                                    <input type="number" id="fldVatPct" name="vat_percent" class="form-control form-control-sm" value="0" min="0" step="0.01" oninput="recalcBill()">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2 px-1">
        ...
        <div class="d-flex gap-2">
            ...
            <button type="submit" class="btn btn-sm btn-primary" id="btnSave">
                <i class="fa fa-save me-1"></i> Save Bill
            </button>
        </div>
    </div>
</div>
...
function addRow(item) {
    ...
    var tr = `<tr data-idx="${i}">
        ...
        <td>
            <input type="number" class="form-control form-control-sm row-demday text-end" value="${demDay.toFixed(2)}" min="0" step="0.01" style="width:75px" oninput="rowDemRecalc(this)">
            <input class="row-demrate" type="hidden" value="${demRate}">
        </td>
        <td><input type="number" class="form-control form-control-sm row-demamt text-end bg-light" value="${demAmt.toFixed(2)}" readonly style="width:85px"></td>
...
$('#btnLoadData').on('click', function () {
    ...
    var customerId = $('#fldCustomerId').val();
    if (!fromDate || !toDate) {
        Swal.fire({ icon: 'warning', title: 'From Date and To Date are required.' });
        return;
    }
    if (!customerId) {
        Swal.fire({ icon: 'warning', title: 'Please select a customer first.' });
        return;
    }
    ...
    .done(function (r) {
        ...
        if (r.customer_address && !$('#fldBillAddress').val()) {
            $('#fldBillAddress').val(r.customer_address);
        }
        if (r.delivery_type) {
            $('#fldDeliveryType').val(r.delivery_type);
        }
        if (r.tds_percent !== undefined) {
            $('#fldTdsPct').val(r.tds_percent);
        }
        if (r.vat_percent !== undefined) {
            $('#fldVatPct').val(r.vat_percent);
        }
        recalcBill();
        $('#billHeaderSection').show();
        if (!r.items || !r.items.length) {
            Swal.fire({ icon: 'info', title: 'No booking items found for selected criteria.' });
        }
    })
    .fail(function (xhr) {
...
$('#btnAddRow').on('click', function () {
    addRow({ booking_date:'', delivery_date:'', item_code:'', item_name:'', location:'', b_qty:1, d_qty:0, due_qty:1, price:0, demurrage_day:0, demurrage_rate:0, demurrage_amount:0, disc_percent:0, discount:0, ait_percent:0, line_amount:0 });
});
...
$('#billForm').on('submit', function (e) {
    ...
    .fail(function (xhr) {
        ...
        $btn.prop('disabled', false).html('<i class="fa fa-save me-1"></i> Save Bill');
    });
});
</script>
@endpush
