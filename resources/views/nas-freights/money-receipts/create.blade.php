...
<div class="page-header">
    <h4><i class="fa fa-money-bill-wave me-2 text-info"></i> New Money Receipt</h4>
...
<div class="row g-3">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header py-2" style="background:#1a6b60;color:#fff;font-size:.82rem;font-weight:600">
                <i class="fa fa-search me-2"></i> Select Customer & Bill
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-5">
                    ...
                    <div class="col-md-5">
                        ...
                        <select id="fldBill" name="bill_id" class="form-select form-select-sm">
                            <option value="">-- Select confirmed bill --</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">&nbsp;</label>
                        <button type="button" id="btnLoadBill" class="btn btn-success btn-sm w-100">
                            <i class="fa fa-sync me-1"></i> Load Bills
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="col-12" id="billInfoSection" style="display:none">
        <div class="card border-success">
            <div class="card-header py-2" style="background:#198754;color:#fff;font-size:.82rem;font-weight:600">
                <i class="fa fa-file-invoice-dollar me-2"></i> Bill Information
            </div>
            <div class="card-body">
                <div class="row g-2">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <div class="info-box">
                            ...
                            <div class="value" id="infoCustName">—</div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="info-box">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header py-2" style="background:#0c2340;color:#fff;font-size:.82rem;font-weight:600">
                <i class="fa fa-receipt me-2"></i> Receipt Details
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-2">
    ...
    <div class="col-12 d-flex justify-content-end gap-2">
        <a href="{{ route('nas-freights.money-receipts.index') }}" class="btn btn-secondary btn-sm">Cancel</a>
        <button type="submit" class="btn btn-success btn-sm px-4" id="btnSave">
            <i class="fa fa-save me-1"></i> Save Receipt & Mark Bill Paid
        </button>
    </div>
</div>
</form>
...
$('#fldCustomer').select2({
    theme: 'bootstrap-5', placeholder: 'Search customer...', minimumInputLength: 3, allowClear: true,
    ajax: {
        url: '{{ route('nas-freights.money-receipts.search-customers') }}',
        dataType: 'json', delay: 250,
        data: d => ({ q: d.term }),
        processResults: d => ({ results: d }),
    },
}).on('select2:select', e => $('#fldCustomerId').val(e.params.data.id))
  .on('select2:clear',  () => { $('#fldCustomerId').val(''); clearBillInfo(); });

$('#btnLoadBill').on('click', function () {
    $.getJSON('{{ route('nas-freights.money-receipts.get-bills') }}', { customer_id: $('#fldCustomerId').val() }, function (data) {
        var $s = $('#fldBill').empty().append('<option value="">-- Select confirmed bill --</option>');
        data.forEach(b => $s.append('<option value="' + b.id + '" data-amt="' + b.total_amount + '" data-no="' + b.bill_no + '" data-cust="' + b.customer_name + '">' + b.text + '</option>'));
        clearBillInfo();
    });
});
...
function clearBillInfo() {
    ...
    $('#fldAmountReceived').val('');
}

// Pre-select bill from URL param (coming from due list)
const urlParams = new URLSearchParams(window.location.search);
if (urlParams.get('bill_id')) {
    $.getJSON('{{ route('nas-freights.money-receipts.get-bills') }}', {}, function (data) {
        var target = data.find(b => b.id == urlParams.get('bill_id'));
        if (target) {
            var $s = $('#fldBill').empty().append('<option value="">-- Select confirmed bill --</option>');
            data.forEach(b => $s.append('<option value="' + b.id + '" data-amt="' + b.total_amount + '" data-no="' + b.bill_no + '" data-cust="' + b.customer_name + '">' + b.text + '</option>'));
...
$('#receiptForm').on('submit', function (e) {
    e.preventDefault();
    if (!$('[name="bill_id"]').val()) { Swal.fire({ icon: 'warning', title: 'Please select a bill.' }); return; }

    const data = $(this).serialize();
    $('#btnSave').prop('disabled', true).html('<i class="fa fa-spinner fa-spin me-1"></i> Saving...');

    $.ajax({
        url: '{{ route('nas-freights.money-receipts.store') }}',
        method: 'POST', data,
        success: r => { Swal.fire({ icon: 'success', title: r.message, timer: 1500, showConfirmButton: false }).then(() => window.location = r.redirect); },
        error: xhr => {
            $('#btnSave').prop('disabled', false).html('<i class="fa fa-save me-1"></i> Save Receipt & Mark Bill Paid');
