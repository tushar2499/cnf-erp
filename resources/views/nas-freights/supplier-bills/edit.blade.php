...
@section('title', 'Edit Payment Order — {{ $supplierBill->pay_order_no }}')
...
<div class="page-header">
    <h4><i class="fa fa-edit me-2 text-info"></i> Edit Payment Order — <strong>{{ $supplierBill->pay_order_no }}</strong></h4>
    ...
    <div class="p-3">
        <div class="row g-2 align-items-end">
            <div class="col-md-2">
                ...
                <input type="date" id="fldFromDate" class="form-control form-control-sm" value="{{ $supplierBill->from_date?->format('Y-m-d') }}" required>
            </div>
            <div class="col-md-2">
                <label class="form-label">To Date <span class="req">*</span></label>
                <input type="date" id="fldToDate" class="form-control form-control-sm" value="{{ $supplierBill->to_date?->format('Y-m-d') }}" required>
            </div>
            <div class="col-md-5">
                <label class="form-label">Supplier <span class="text-muted fw-normal">(Optional)</span></label>
                <select id="fldSupplier" class="form-select form-select-sm" style="width:100%"></select>
                <input type="hidden" id="fldSupplierId"   name="supplier_id"   value="{{ $supplierBill->supplier_id }}">
                <input type="hidden" id="fldSupplierName" name="supplier_name" value="{{ $supplierBill->supplier_name }}">
            </div>
            <div class="col-md-3">
        ...
        <div class="p-3">
            <div class="row g-2">
                <div class="col-md-3">
                    ...
                    <input type="date" id="fldBillDate" name="bill_date" class="form-control form-control-sm" value="{{ $supplierBill->bill_date?->format('Y-m-d') }}" required>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <textarea id="fldNote" name="note" class="form-control form-control-sm" rows="1">{{ $supplierBill->note }}</textarea>
                </div>
            </div>
        </div>
    </div>
...
/* ── Pre-initialize Supplier Select2 ── */
@if($supplierBill->supplier_id)
$('#fldSupplier').append(new Option(@json($supplierBill->supplier_name), @json($supplierBill->supplier_id), true, true)).trigger('change');
@endif
$('#fldSupplier').select2({
    theme: 'bootstrap-5',
...
@if($supplierBill->bill_by)
$('#fldBillBy').append(new Option(@json($supplierBill->bill_by), @json($supplierBill->bill_by), true, true)).trigger('change');
@endif
$('#fldBillBy').select2({
    theme: 'bootstrap-5',
...
function rowRecalc(el) {
    ...
    $tr.find('.row-lineamt').val((bQty * price + demAmt).toFixed(2));
    recalcTotal();
}

function recalcTotal() {
    var total = 0;
    $('#rowsBody tr').each(function () { total += parseFloat($(this).find('.row-lineamt').val()) || 0; });
...
$(document).on('click', '.btn-del-row', function () {
    $(this).closest('tr').remove(); reindex(); recalcTotal();
});
...
$('#btnLoadData').on('click', function () {
    if (!$('#fldFromDate').val() || !$('#fldToDate').val()) {
        Swal.fire({ icon: 'warning', title: 'From Date and To Date are required.' }); return;
    }
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { _token: CSRF, from_date: $('#fldFromDate').val(), to_date: $('#fldToDate').val(), supplier_id: $('#fldSupplierId').val() },
    })
    .done(function (r) {
        $('#rowsBody').empty(); rowIdx = 0;
        if (r.items && r.items.length) r.items.forEach(item => addRow(item));
        else Swal.fire({ icon: 'info', title: 'No booking items found.' });
    })
