...
<div class="row g-3">
    <div class="col-md-4">
        <div class="panel">
            <div class="panel-header" id="formTitle"><i class="fa fa-plus me-2"></i> Add New Port</div>
            <div class="p-3">
                <form id="mainForm">
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-6"><label class="form-label">Country</label><input type="text" id="fCountry" class="form-control form-control-sm"></div>
                        <div class="col-6"><label class="form-label">Type <span class="text-danger">*</span></label>
                            <select id="fType" class="form-select form-select-sm"><option value="Sea">Sea</option><option value="Air">Air</option><option value="Land">Land</option></select>
                        </div>
                    </div>
    ...
    <div class="col-md-8">
        <div class="panel">
            <div class="panel-header"><i class="fa fa-list me-2"></i> Port List</div>
            <div style="overflow-x:auto">
                <table id="mainTable" class="table table-hover table-striped dt-table mb-0 w-100">
                    <thead><tr><th>#</th><th>Name</th><th>Country</th><th>Type</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody></tbody>
                </table>
            </div>
        </div>
...
$(function () {
    table = $('#mainTable').DataTable({
        processing: true, serverSide: true, ajax: '{{ route('nas-trading.ports.index') }}',
        columns: [
            { data: 'DT_RowIndex', orderable: false, searchable: false, width: '40px' },
            { data: 'name', name: 'name' }, { data: 'country', name: 'country' },
    ...
    function resetForm() {
        $('#recId').val(''); $('#fName,#fCountry').val('');
        $('#fName').removeClass('is-invalid'); $('#fNameErr').text('');
        $('#fType').val('Sea'); $('#fStatus').val('Active');
        $('#formTitle').html('<i class="fa fa-plus me-2"></i> Add New Port');
        $('#btnSave').html('<i class="fa fa-save me-1"></i> Save');
    }
    $('#btnAddNew,#btnCancel').on('click', resetForm);
    $(document).on('click', '.btn-edit', function () {
        $.getJSON('{{ url('nas-trading/ports') }}/' + $(this).data('id'), function (r) {
            resetForm(); $('#recId').val(r.id); $('#fName').val(r.name); $('#fCountry').val(r.country||''); $('#fType').val(r.type); $('#fStatus').val(r.status);
            $('#formTitle').html('<i class="fa fa-edit me-2"></i> Edit Port'); $('#btnSave').html('<i class="fa fa-save me-1"></i> Update');
    ...
    $('#mainForm').on('submit', function (e) {
        ...
        const id = $('#recId').val(), url = id ? '{{ url('nas-trading/ports') }}/' + id : '{{ route('nas-trading.ports.store') }}';
        $('#btnSave').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Saving...');
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: { _token: $('meta[name="csrf-token"]').attr('content'), name: $('#fName').val(), country: $('#fCountry').val(), type: $('#fType').val(), status: $('#fStatus').val() }})
