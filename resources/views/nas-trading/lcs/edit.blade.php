    ...
    <form id="lcForm">
                ...
                <div class="row g-2">
                    <div class="col-md-3">
                        ...
                        <input type="text" class="form-control form-control-sm bg-light fw-bold"
                            value="{{ $lc->lc_no_system }}" readonly>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="lc_type" class="form-select form-select-sm">
                            ...
                            @foreach (['TT/LCA', 'Sight', 'DF'] as $t)
                                <option value="{{ $t }}" {{ $lc->lc_type === $t ? 'selected' : '' }}>
                                    {{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="lc_status" class="form-select form-select-sm">
                            @foreach (['Open', 'Closed', 'Cancelled', 'Amended'] as $s)
                                <option value="{{ $s }}" {{ $lc->lc_status === $s ? 'selected' : '' }}>
                                    {{ $s }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="last_shipment_date" class="form-control form-control-sm"
                            value="{{ $lc->last_shipment_date?->format('Y-m-d') }}">
                    </div>
                    <div class="col-md-3">
                        ...
                        <input type="date" name="shipping_docs_received_date" class="form-control form-control-sm"
                            value="{{ $lc->shipping_docs_received_date?->format('Y-m-d') }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lc-card mt-1">
            <div class="lc-section-header"><i class="fa fa-list-alt me-2"></i> Additional Details <span
                    class="fw-normal ms-2" style="font-size:.72rem;opacity:.8">(optional)</span></div>
            <div class="px-3 pt-2">
                    ...
                    <div class="row g-2 mt-1">
                            ...
                            <div class="input-group input-group-sm">
                                <select name="importer_id" id="importerSelect" class="form-select form-select-sm">
                                    ...
                                    @foreach ($importers as $imp)
                                        <option value="{{ $imp->id }}"
                                            {{ $lc->importer_id == $imp->id ? 'selected' : '' }}>{{ $imp->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="customer_po_date" class="form-control form-control-sm"
                                value="{{ $lc->customer_po_date?->format('Y-m-d') }}">
                        </div>
                    </div>
                </div>
                            ...
                            <select name="opening_bank_id" class="form-select form-select-sm">
                                ...
                                @foreach ($banks as $bank)
                                    <option value="{{ $bank->id }}"
                                        {{ $lc->opening_bank_id == $bank->id ? 'selected' : '' }}>
                            ...
                            <select name="port_of_dest_id" class="form-select form-select-sm">
                                ...
                                @foreach ($ports as $port)
                                    <option value="{{ $port->id }}"
                                        {{ $lc->port_of_dest_id == $port->id ? 'selected' : '' }}>{{ $port->name }}
                                        ({{ $port->type }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="psi_company_id" class="form-select form-select-sm">
                                ...
                                @foreach ($psiCompanies as $psi)
                                    <option value="{{ $psi->id }}"
                                        {{ $lc->psi_company_id == $psi->id ? 'selected' : '' }}>{{ $psi->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select name="doc_status" class="form-select form-select-sm">
                                @foreach (['Pending', 'Received', 'Complete'] as $s)
                                    <option value="{{ $s }}" {{ $lc->doc_status === $s ? 'selected' : '' }}>
                                        {{ $s }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row g-2 mt-1">
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="pfi_value" id="pfiValue"
                                    ...
                                    value="{{ fmtNum($lc->pfi_value) }}" placeholder="0.00">
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <select name="currency" id="currency" class="form-select form-select-sm">
                                @foreach (['USD', 'EUR', 'GBP', 'CNY'] as $c)
                                    <option value="{{ $c }}" {{ $lc->currency === $c ? 'selected' : '' }}>
                                        {{ $c }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="lc_open_rate" id="lcOpRate"
                                    class="form-control form-control-sm" step="0.0001"
                                    value="{{ fmtNum($lc->lc_open_rate) }}" placeholder="0.00">
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="margin_percent" id="marginPct"
                                    class="form-control form-control-sm" step="0.0001"
                                    value="{{ fmtNum($lc->margin_percent) }}" placeholder="0.00">
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="freight_value" id="freightValue"
                                    class="form-control form-control-sm" step="0.0001"
                                    value="{{ fmtNum($lc->freight_value) }}" placeholder="0.00">
                                <span class="input-group-text fcy-label">USD</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="lc_value" id="lcValue"
                                    ...
                                    value="{{ fmtNum($lc->lc_value) }}" placeholder="0.00">
                                <span class="input-group-text fcy-label">USD</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="doc_rt_rate" class="form-control form-control-sm"
                                    step="0.0001" value="{{ fmtNum($lc->doc_rt_rate) }}" placeholder="0.00">
                                <span class="input-group-text">BDT</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="lc_rt_value" id="lcRtValue"
                                    ...
                                    value="{{ fmtNum($lc->lc_rt_value) }}" placeholder="0.00">
                                <span class="input-group-text">BDT</span>
                            </div>
                        </div>
                        <div class="col-6 col-md-3">
                            ...
                            <div class="input-group input-group-sm mb-1">
                                <input type="number" name="lc_commission_percent" id="lcCommissionPct"
                                    ...
                                    value="{{ fmtNum($lc->lc_commission_percent) }}" placeholder="0"
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <input type="text" name="lc_charge_posting" class="form-control form-control-sm"
                                value="{{ $lc->lc_charge_posting }}" placeholder="e.g. DR-001">
                        </div>
                        ...
                        <div class="col-6 col-md-3">
                            ...
                            <input type="date" name="insurance_validity" class="form-control form-control-sm"
                                value="{{ $lc->insurance_validity?->format('Y-m-d') }}">
                        </div>
                        <div class="col-6 col-md-3">
                            ...
                            <select name="comm_currency" class="form-select form-select-sm">
                                @foreach (['USD', 'EUR', 'GBP', 'CNY', 'BDT'] as $c)
                                    <option value="{{ $c }}"
                                        {{ $lc->comm_currency === $c ? 'selected' : '' }}>{{ $c }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-6 col-md-3">
                    ...
                    <div class="row g-2 mt-1">
                        <div class="col-md-3">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="duty_advance" class="form-control form-control-sm"
                                    step="0.01" value="{{ fmtNum($lc->duty_advance) }}" placeholder="0.00">
                                <span class="input-group-text">BDT</span>
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="date" name="duty_advance_date" class="form-control form-control-sm"
                                value="{{ $lc->duty_advance_date?->format('Y-m-d') }}">
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="text" name="duty_advance_posting" class="form-control form-control-sm"
                                value="{{ $lc->duty_advance_posting }}" placeholder="e.g. DA-001">
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="bill_of_entry_no" class="form-control form-control-sm"
                                value="{{ $lc->bill_of_entry_no }}" placeholder="e.g. BE-2025-001">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="cnf_cost_posting" class="form-control form-control-sm"
                                value="{{ $lc->cnf_cost_posting }}" placeholder="e.g. CC-001">
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-1 mb-2">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="lc_closing_bill_date" class="form-control form-control-sm"
                                value="{{ $lc->lc_closing_bill_date?->format('Y-m-d') }}">
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-1">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="received_date" class="form-control form-control-sm"
                                value="{{ $lc->received_date?->format('Y-m-d') }}">
                        </div>
                        <div class="col-md-3"></div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="vat_return_date" class="form-control form-control-sm"
                                value="{{ $lc->vat_return_date?->format('Y-m-d') }}">
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="text" name="vat_return_posting" class="form-control form-control-sm"
                                value="{{ $lc->vat_return_posting }}" placeholder="e.g. VR-001">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="lc_commission_date" class="form-control form-control-sm"
                                value="{{ $lc->lc_commission_date?->format('Y-m-d') }}">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="sales_posting" class="form-control form-control-sm"
                                value="{{ $lc->sales_posting }}" placeholder="e.g. SP-001">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="coss_posting" class="form-control form-control-sm"
                                value="{{ $lc->coss_posting }}" placeholder="e.g. CP-001">
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function fmtPrice(v) {
            return (v !== null && v !== '' && v !== undefined) ? parseFloat(v).toFixed(2) : '';
        }
    ...
    <tr>
        ...
        <td><input type="number" class="form-control form-control-sm item-qty" name="items[${idx}][qty_count]" value="${fmtQty(data.qty_count)}" step="0.0001" min="0" placeholder="0"></td>
        ...
        $(function() {
            ...
            }).on('select2:select', e => $('#customerName').val(e.params.data.text.split(' | ')[1] || e.params
                .data.text));
            ...
            $(document).on('input', '.item-qty, .item-uprice', function() {
                var row = $(this).closest('tr');
                row.find('.item-amount').val(((parseFloat(row.find('.item-qty').val()) || 0) * (parseFloat(
                    row.find('.item-uprice').val()) || 0)).toFixed(2));
            });
            ...
            $('#lcForm').on('submit', function(e) {
                ...
                $.ajax({
                        url: '{{ route('nas-trading.lcs.update', $lc->id) }}',
                        method: 'POST',
                    ...
                    .fail(xhr => {
                        ...
                        if (xhr.status === 422) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: xhr.responseJSON?.message || 'Error saving LC.'
                            });
                        }
                    });
            });
            ...
            $('#btnSaveImporter').on('click', function() {
                ...
                $.ajax({
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: xhr.responseJSON?.message || 'Failed to save importer.'
                    });
                }).always(function() {
