...
#importerTable_wrapper>.row:last-child {
...
<div class="page-header">
    ...
    <button class="btn btn-sm btn-info text-white" data-bs-toggle="modal" data-bs-target="#importerModal" id="btnAddNew">
...
<div class="panel">
    <div class="panel-header d-flex justify-content-between align-items-center">
        <span><i class="fa fa-list me-2"></i> Importer List</span>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item" onclick="$('#importerTable').DataTable().button('.buttons-csv').trigger()"><i class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item" onclick="$('#importerTable').DataTable().button('.buttons-excel').trigger()"><i class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item" onclick="$('#importerTable').DataTable().button('.buttons-pdf').trigger()"><i class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item" onclick="$('#importerTable').DataTable().button('.buttons-print').trigger()"><i class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="dt-scroll">
        <table id="importerTable" class="table table-hover table-striped table-bordered dt-table mb-0 w-100">
            <thead>
            <tr>
                <th>#</th><th>Name</th><th>BIN No</th><th>Address</th><th>Status</th><th>Action</th>
            </tr>
            <tr>
                <th></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search Name"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search BIN No"></th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title fw-600" id="importerModalTitle"><i class="fa fa-plus me-2"></i> Add New Importer</h6>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="fBinNo" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
...
$(function () {
    table = $('#importerTable').DataTable({
        ...
        language: { emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No importers yet.</div>' },
        initComplete: function () {
            const firstRowH = $('#importerTable thead tr:first-child').outerHeight();
            $('#importerTable thead tr:last-child th').css('top', firstRowH + 'px');
    ...
    function resetForm() {
        $('#recId').val('');
        $('#fName,#fBinNo,#fAddress').val('');
        $('#fName').removeClass('is-invalid'); $('#fNameErr').text('');
        $('#fStatus').val('Active');
        $('#importerModalTitle').html('<i class="fa fa-plus me-2"></i> Add New Importer');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.getJSON('{{ url('nas-trading/importers') }}/' + id, function (r) {
            resetForm();
            $('#recId').val(r.id); $('#fName').val(r.name); $('#fBinNo').val(r.bin_no || ''); $('#fAddress').val(r.address || ''); $('#fStatus').val(r.status);
            $('#importerModalTitle').html('<i class="fa fa-edit me-2"></i> Edit Importer');
            ...
            .then(res => {
                if (res.isConfirmed) {
                    $.ajax({ url, method: 'DELETE', data: { _token: $('meta[name="csrf-token"]').attr('content') } })
                        .done(r => { Swal.fire({ icon: 'success', title: r.message || 'Importer deleted.', timer: 1500, showConfirmButton: false }); table.ajax.reload(); })
                        .fail(() => Swal.fire({ icon: 'error', title: 'Delete failed.' }));
                }
            });
    });

    $('#importerForm').on('submit', function (e) {
        e.preventDefault();
        $('#fName').removeClass('is-invalid'); $('#fNameErr').text('');
        if (!$('#fName').val().trim()) { $('#fName').addClass('is-invalid'); $('#fNameErr').text('Name is required.'); return; }
        const id = $('#recId').val();
        const url = id ? '{{ url('nas-trading/importers') }}/' + id : '{{ route('nas-trading.importers.store') }}';
        ...
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: {
            ...
            name: $('#fName').val(), bin_no: $('#fBinNo').val(), address: $('#fAddress').val(), status: $('#fStatus').val(),
        }})
            .done(function (r) {
                $('#importerModal').modal('hide');
                Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: r.message || 'Importer saved.', showConfirmButton: false, timer: 2500, timerProgressBar: true });
