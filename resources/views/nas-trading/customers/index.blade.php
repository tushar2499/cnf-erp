...
#customerTable_wrapper>.row:last-child {
    ...
    <div class="panel-header d-flex justify-content-between align-items-center">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item" onclick="$('#customerTable').DataTable().button('.buttons-csv').trigger()"><i class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item" onclick="$('#customerTable').DataTable().button('.buttons-excel').trigger()"><i class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item" onclick="$('#customerTable').DataTable().button('.buttons-pdf').trigger()"><i class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item" onclick="$('#customerTable').DataTable().button('.buttons-print').trigger()"><i class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="dt-scroll">
        <table id="customerTable" class="table table-hover table-striped table-bordered dt-table mb-0 w-100">
            <thead>
            <tr>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title fw-600" id="customerModalTitle"><i class="fa fa-plus me-2"></i> Add New Customer</h6>
...
$(function () {
    table = $('#customerTable').DataTable({
        ...
        initComplete: function () {
            const firstRowH = $('#customerTable thead tr:first-child').outerHeight();
            $('#customerTable thead tr:last-child th').css('top', firstRowH + 'px');
    ...
    function resetForm() {
        ...
        $('#fCompanyName').removeClass('is-invalid');
        $('#fNameErr').text('');
        $('#fCreditLimit').val(0);
        $('#fStatus').val('Active');
        $('#customerModalTitle').html('<i class="fa fa-plus me-2"></i> Add New Customer');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.getJSON('{{ url('nas-trading/customers') }}/' + id, function (r) {
            ...
            $('#recId').val(r.id);
            $('#fCode').val(r.code);
            $('#fCompanyName').val(r.company_name);
            $('#fContactPerson').val(r.contact_person || '');
            $('#fPhone').val(r.phone || '');
            $('#fEmail').val(r.email || '');
            $('#fAddress').val(r.address || '');
            $('#fDeliveryAddress').val(r.delivery_address || '');
            $('#fCreditLimit').val(r.credit_limit || 0);
            $('#fStatus').val(r.status);
            $('#customerModalTitle').html('<i class="fa fa-edit me-2"></i> Edit Customer');
    ...
    $('#customerForm').on('submit', function (e) {
        ...
        $('#fCompanyName').removeClass('is-invalid');
        $('#fNameErr').text('');
        if (!$('#fCompanyName').val().trim()) {
            $('#fCompanyName').addClass('is-invalid');
            $('#fNameErr').text('Company name is required.');
            return;
        }
        const id = $('#recId').val();
        ...
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: {
            ...
            company_name: $('#fCompanyName').val(),
            contact_person: $('#fContactPerson').val(),
            phone: $('#fPhone').val(),
            email: $('#fEmail').val(),
            address: $('#fAddress').val(),
            delivery_address: $('#fDeliveryAddress').val(),
            credit_limit: $('#fCreditLimit').val(),
            status: $('#fStatus').val(),
        }})
            .done(function (r) {
            ...
            .fail(function (xhr) {
                if (xhr.status === 422) {
                    ...
                    if (e.company_name) { $('#fCompanyName').addClass('is-invalid'); $('#fNameErr').text(e.company_name[0]); }
