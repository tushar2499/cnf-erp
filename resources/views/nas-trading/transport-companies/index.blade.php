...
<div class="row g-3">
    <div class="col-md-4">
        <div class="panel">
            <div class="panel-header" id="formTitle"><i class="fa fa-plus me-2"></i> Add Transport Company</div>
            <div class="p-3">
                <form id="mainForm">
    ...
    <div class="col-md-8">
        <div class="panel">
            <div class="panel-header"><i class="fa fa-list me-2"></i> Transport Company List</div>
...
$(function () {
    table = $('#mainTable').DataTable({
        processing: true, serverSide: true, ajax: '{{ route('nas-trading.transport-companies.index') }}',
        columns: [
    ...
    function resetForm() {
        ...
        $('#fStatus').val('Active');
        $('#formTitle').html('<i class="fa fa-plus me-2"></i> Add Transport Company');
        $('#btnSave').html('<i class="fa fa-save me-1"></i> Save');
    }
    $('#btnAddNew,#btnCancel').on('click', resetForm);
    $(document).on('click', '.btn-edit', function () {
        $.getJSON('{{ url('nas-trading/transport-companies') }}/' + $(this).data('id'), function (r) {
            resetForm(); $('#recId').val(r.id); $('#fName').val(r.name); $('#fPhone').val(r.phone||''); $('#fAddress').val(r.address||''); $('#fStatus').val(r.status);
            $('#formTitle').html('<i class="fa fa-edit me-2"></i> Edit Transport Company'); $('#btnSave').html('<i class="fa fa-save me-1"></i> Update');
    ...
    $('#mainForm').on('submit', function (e) {
        ...
        const id = $('#recId').val(), url = id ? '{{ url('nas-trading/transport-companies') }}/' + id : '{{ route('nas-trading.transport-companies.store') }}';
