...
#transportCompanyTable_wrapper>.row:last-child {
...
<div class="page-header">
    <h4><i class="fa fa-truck-moving me-2 text-info"></i> Transport Companies</h4>
    <button class="btn btn-sm btn-info text-white" data-bs-toggle="modal" data-bs-target="#transportCompanyModal" id="btnAddNew">
...
<div class="panel">
    <div class="panel-header d-flex justify-content-between align-items-center">
        <span><i class="fa fa-list me-2"></i> Transport Company List</span>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item" onclick="$('#transportCompanyTable').DataTable().button('.buttons-csv').trigger()"><i class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item" onclick="$('#transportCompanyTable').DataTable().button('.buttons-excel').trigger()"><i class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item" onclick="$('#transportCompanyTable').DataTable().button('.buttons-pdf').trigger()"><i class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item" onclick="$('#transportCompanyTable').DataTable().button('.buttons-print').trigger()"><i class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="dt-scroll">
        <table id="transportCompanyTable" class="table table-hover table-striped table-bordered dt-table mb-0 w-100">
            <thead>
            <tr>
...
<div class="modal fade" id="transportCompanyModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title fw-600" id="transportCompanyModalTitle"><i class="fa fa-plus me-2"></i> Add Transport Company</h6>
...
var table;
$(function () {
    table = $('#transportCompanyTable').DataTable({
        ...
        ajax: '{{ route('nas-trading.transport-companies.index') }}',
        columns: [
        ...
        language: { emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No transport companies yet.</div>' },
        initComplete: function () {
            const firstRowH = $('#transportCompanyTable thead tr:first-child').outerHeight();
            $('#transportCompanyTable thead tr:last-child th').css('top', firstRowH + 'px');
    ...
    function resetForm() {
        ...
        $('#fStatus').val('Active');
        $('#transportCompanyModalTitle').html('<i class="fa fa-plus me-2"></i> Add Transport Company');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.getJSON('{{ url('nas-trading/transport-companies') }}/' + id, function (r) {
            resetForm();
            $('#recId').val(r.id); $('#fName').val(r.name); $('#fPhone').val(r.phone || ''); $('#fAddress').val(r.address || ''); $('#fStatus').val(r.status);
            $('#transportCompanyModalTitle').html('<i class="fa fa-edit me-2"></i> Edit Transport Company');
    ...
    $('#transportCompanyForm').on('submit', function (e) {
        ...
        const id = $('#recId').val();
        const url = id ? '{{ url('nas-trading/transport-companies') }}/' + id : '{{ route('nas-trading.transport-companies.store') }}';
