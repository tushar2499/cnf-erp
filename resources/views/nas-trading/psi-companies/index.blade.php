...
#psiCompanyTable_wrapper>.row:last-child {
...
<div class="page-header">
    <h4><i class="fa fa-search me-2 text-info"></i> PSI Companies</h4>
    <button class="btn btn-sm btn-info text-white" data-bs-toggle="modal" data-bs-target="#psiCompanyModal" id="btnAddNew">
...
<div class="panel">
    <div class="panel-header d-flex justify-content-between align-items-center">
        <span><i class="fa fa-list me-2"></i> PSI Company List</span>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item" onclick="$('#psiCompanyTable').DataTable().button('.buttons-csv').trigger()"><i class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item" onclick="$('#psiCompanyTable').DataTable().button('.buttons-excel').trigger()"><i class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item" onclick="$('#psiCompanyTable').DataTable().button('.buttons-pdf').trigger()"><i class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item" onclick="$('#psiCompanyTable').DataTable().button('.buttons-print').trigger()"><i class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="dt-scroll">
        <table id="psiCompanyTable" class="table table-hover table-striped table-bordered dt-table mb-0 w-100">
            <thead>
            <tr>
                <th>#</th><th>Name</th><th>Country</th><th>Status</th><th>Action</th>
            </tr>
            <tr>
                <th></th>
...
<div class="modal fade" id="psiCompanyModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title fw-600" id="psiCompanyModalTitle"><i class="fa fa-plus me-2"></i> Add PSI Company</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="psiCompanyForm">
                @csrf
...
var table;
$(function () {
    table = $('#psiCompanyTable').DataTable({
        ...
        language: { emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No PSI companies yet.</div>' },
        initComplete: function () {
            const firstRowH = $('#psiCompanyTable thead tr:first-child').outerHeight();
            $('#psiCompanyTable thead tr:last-child th').css('top', firstRowH + 'px');
    ...
    function resetForm() {
        ...
        $('#fStatus').val('Active');
        $('#psiCompanyModalTitle').html('<i class="fa fa-plus me-2"></i> Add PSI Company');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.getJSON('{{ url('nas-trading/psi-companies') }}/' + id, function (r) {
            resetForm();
            $('#recId').val(r.id); $('#fName').val(r.name); $('#fCountry').val(r.country || ''); $('#fStatus').val(r.status);
            $('#psiCompanyModalTitle').html('<i class="fa fa-edit me-2"></i> Edit PSI Company');
            $('#btnSave').html('<i class="fa fa-save me-1"></i> Update');
            $('#psiCompanyModal').modal('show');
        });
    });
    ...
    $('#psiCompanyForm').on('submit', function (e) {
        e.preventDefault();
        $('#fName').removeClass('is-invalid'); $('#fNameErr').text('');
        if (!$('#fName').val().trim()) { $('#fName').addClass('is-invalid'); $('#fNameErr').text('Name is required.'); return; }
        const id = $('#recId').val();
        const url = id ? '{{ url('nas-trading/psi-companies') }}/' + id : '{{ route('nas-trading.psi-companies.store') }}';
        ...
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: {
            ...
            name: $('#fName').val(), country: $('#fCountry').val(), status: $('#fStatus').val(),
        }})
            .done(function (r) {
                $('#psiCompanyModal').modal('hide');
