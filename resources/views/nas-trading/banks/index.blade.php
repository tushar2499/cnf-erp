...
#bankTable_wrapper>.row:last-child {
...
<div class="page-header">
    <h4><i class="fa fa-university me-2 text-info"></i> Banks</h4>
    <button class="btn btn-sm btn-info text-white" data-bs-toggle="modal" data-bs-target="#bankModal" id="btnAddNew">
...
<div class="panel">
    <div class="panel-header d-flex justify-content-between align-items-center">
        <span><i class="fa fa-list me-2"></i> Bank List</span>
        <div class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li><button class="dropdown-item" onclick="$('#bankTable').DataTable().button('.buttons-csv').trigger()"><i class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                <li><button class="dropdown-item" onclick="$('#bankTable').DataTable().button('.buttons-excel').trigger()"><i class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                <li><button class="dropdown-item" onclick="$('#bankTable').DataTable().button('.buttons-pdf').trigger()"><i class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                <li>
                    <hr class="dropdown-divider">
                </li>
                <li><button class="dropdown-item" onclick="$('#bankTable').DataTable().button('.buttons-print').trigger()"><i class="fa fa-print me-2"></i>Print</button></li>
            </ul>
        </div>
    </div>
    <div class="dt-scroll">
        <table id="bankTable" class="table table-hover table-striped table-bordered dt-table mb-0 w-100">
            <thead>
            <tr>
                <th>#</th><th>Name</th><th>Branch</th><th>SWIFT</th><th>Account No</th><th>Status</th><th>Action</th>
            </tr>
            <tr>
                <th></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search Name"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search Branch"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search SWIFT"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Search Account No"></th>
                <th></th>
                <th></th>
            </tr>
            </thead>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title fw-600" id="bankModalTitle"><i class="fa fa-plus me-2"></i> Add New Bank</h6>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="fBranch" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">SWIFT Code</label>
                            <input type="text" id="fSwiftCode" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Account No.</label>
                            <input type="text" id="fAccountNo" class="form-control form-control-sm">
                        </div>
                        <div class="col-md-6">
...
$(function () {
    table = $('#bankTable').DataTable({
        ...
        language: { emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No banks yet.</div>' },
        initComplete: function () {
            const firstRowH = $('#bankTable thead tr:first-child').outerHeight();
            $('#bankTable thead tr:last-child th').css('top', firstRowH + 'px');
    ...
    function resetForm() {
        $('#recId').val(''); $('#fName,#fBranch,#fSwiftCode,#fAccountNo').val('');
        $('#fName').removeClass('is-invalid'); $('#fNameErr').text('');
        $('#fStatus').val('Active');
        $('#bankModalTitle').html('<i class="fa fa-plus me-2"></i> Add New Bank');
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.getJSON('{{ url('nas-trading/banks') }}/' + id, function (r) {
            resetForm();
            $('#recId').val(r.id); $('#fName').val(r.name); $('#fBranch').val(r.branch || '');
            $('#fSwiftCode').val(r.swift_code || ''); $('#fAccountNo').val(r.account_no || ''); $('#fStatus').val(r.status);
            $('#bankModalTitle').html('<i class="fa fa-edit me-2"></i> Edit Bank'); $('#btnSave').html('<i class="fa fa-save me-1"></i> Update');
    ...
    $('#bankForm').on('submit', function (e) {
        ...
        const id = $('#recId').val();
        const url = id ? '{{ url('nas-trading/banks') }}/' + id : '{{ route('nas-trading.banks.store') }}';
        ...
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: {
            ...
            name: $('#fName').val(), branch: $('#fBranch').val(), swift_code: $('#fSwiftCode').val(), account_no: $('#fAccountNo').val(), status: $('#fStatus').val(),
        }})
            .done(function (r) {
