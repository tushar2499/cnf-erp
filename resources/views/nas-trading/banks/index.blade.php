...
<div class="row g-3">
    <div class="col-md-4">
        <div class="panel">
            <div class="panel-header" id="formTitle"><i class="fa fa-plus me-2"></i> Add New Bank</div>
            <div class="p-3">
                <form id="mainForm">
                    ...
                    <div class="mb-2"><label class="form-label">Branch</label><input type="text" id="fBranch" class="form-control form-control-sm"></div>
                    <div class="mb-2"><label class="form-label">SWIFT Code</label><input type="text" id="fSwiftCode" class="form-control form-control-sm"></div>
                    <div class="mb-2"><label class="form-label">Account No.</label><input type="text" id="fAccountNo" class="form-control form-control-sm"></div>
    ...
    <div class="col-md-8">
        <div class="panel">
            <div class="panel-header"><i class="fa fa-list me-2"></i> Bank List</div>
            <div style="overflow-x:auto">
                <table id="mainTable" class="table table-hover table-striped dt-table mb-0 w-100">
                    <thead><tr><th>#</th><th>Name</th><th>Branch</th><th>SWIFT</th><th>Account No</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody></tbody>
                </table>
            </div>
        </div>
...
$(function () {
    table = $('#mainTable').DataTable({
        processing: true, serverSide: true, ajax: '{{ route('nas-trading.banks.index') }}',
        columns: [
            { data: 'DT_RowIndex', orderable: false, searchable: false, width: '40px' },
            { data: 'name', name: 'name' }, { data: 'branch', name: 'branch' },
            { data: 'swift_code', name: 'swift_code' }, { data: 'account_no', name: 'account_no' },
    ...
    function resetForm() {
        ...
        $('#fStatus').val('Active');
        $('#formTitle').html('<i class="fa fa-plus me-2"></i> Add New Bank');
        $('#btnSave').html('<i class="fa fa-save me-1"></i> Save');
    }
    $('#btnAddNew,#btnCancel').on('click', resetForm);
    $(document).on('click', '.btn-edit', function () {
        $.getJSON('{{ url('nas-trading/banks') }}/' + $(this).data('id'), function (r) {
            resetForm(); $('#recId').val(r.id); $('#fName').val(r.name); $('#fBranch').val(r.branch||'');
            $('#fSwiftCode').val(r.swift_code||''); $('#fAccountNo').val(r.account_no||''); $('#fStatus').val(r.status);
            $('#formTitle').html('<i class="fa fa-edit me-2"></i> Edit Bank'); $('#btnSave').html('<i class="fa fa-save me-1"></i> Update');
    ...
    $('#mainForm').on('submit', function (e) {
        ...
        const id = $('#recId').val(), url = id ? '{{ url('nas-trading/banks') }}/' + id : '{{ route('nas-trading.banks.store') }}';
        $('#btnSave').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Saving...');
        $.ajax({ url, method: id ? 'PUT' : 'POST', data: { _token: $('meta[name="csrf-token"]').attr('content'), name: $('#fName').val(), branch: $('#fBranch').val(), swift_code: $('#fSwiftCode').val(), account_no: $('#fAccountNo').val(), status: $('#fStatus').val() }})
