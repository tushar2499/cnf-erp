...
<body>
    <div class="no-print">
        <button class="btn-act btn-act-print" onclick="window.print()"><svg xmlns="http://www.w3.org/2000/svg"
    ...
    @php
        ...
        function lcClosingNumberToWords(float $amount): string
        {
            ...
            $convert = function (int $n) use (&$convert, $ones, $tens): string {
                ...
                if ($n < 1000) {
                    return $ones[intdiv($n, 100)] . ' Hundred' . ($n % 100 ? ' ' . $convert($n % 100) : '');
                }
                if ($n < 100000) {
                    return $convert(intdiv($n, 1000)) . ' Thousand' . ($n % 1000 ? ' ' . $convert($n % 1000) : '');
                }
                if ($n < 10000000) {
                    return $convert(intdiv($n, 100000)) . ' Lakh' . ($n % 100000 ? ' ' . $convert($n % 100000) : '');
                }
    ...
    <div class="doc">
        ...
        <table>
            ...
            <tfoot>
                <tr>
                    <td colspan="2" class="text-right" style="border-left:1px solid #000;font-weight:bold;">Total
                        Cost BDT.</td>
                ...
                <tr class="dues-row">
                    <td colspan="2" class="text-right" style="border-left:1px solid #000;font-weight:bold;">Dues BDT.
                    </td>
