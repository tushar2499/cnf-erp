...
@push('styles')
    <style>
        ...
        .ro-field {
            background: var(--c-ro-bg) !important;
            color: var(--c-ro-txt);
            cursor: default;
        }
        ...
        .charge-tbl td:first-child {
            white-space: nowrap;
            font-weight: 500;
            color: #374151;
        }

        .charge-tbl tbody tr:hover td {
            background: #f8fafc;
        }

        .charge-tbl input.form-control-sm {
            font-size: .775rem;
            padding: .2rem .35rem;
            height: auto;
        }
...
@section('content')
    ...
    <div class="page-hdr">
        <div class="d-flex align-items-center gap-2">
            ...
            @if ($job)
                <span class="badge bg-primary ms-1">{{ $job->job_no }}</span>
            @endif
        </div>
    ...
    <form method="POST" action="{{ $job ? route('chevron.cnf.jobs.update', $job->id) : route('chevron.cnf.jobs.store') }}">
        ...
        <div class="row g-3 align-items-start">
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <select name="job_type_id" class="form-select form-select-sm" required>
                                    ...
                                    @foreach ($jobTypes as $jt)
                                        <option value="{{ $jt->id }}"
                                            {{ old('job_type_id', $job?->job_type_id) == $jt->id ? 'selected' : '' }}>
                                            {{ $jt->name }}</option>
                                    @endforeach
                                </select>
                                <small class="text-danger d-none" id="job_type_id_err">The job type field is
                                    required.</small>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="port_id" class="form-select form-select-sm" required>
                                    ...
                                    @foreach ($ports as $p)
                                        <option value="{{ $p->id }}"
                                            {{ old('port_id', $job?->port_id) == $p->id ? 'selected' : '' }}>
                                            {{ $p->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="country_of_origin" class="form-select form-select-sm">
                                    ...
                                    @foreach ($countries as $c)
                                        <option value="{{ $c }}"
                                            {{ old('country_of_origin', $job?->country_of_origin) === $c ? 'selected' : '' }}>
                                            {{ $c }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="date" name="job_date" class="form-control form-control-sm"
                                    value="{{ old('job_date', $job?->job_date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}" required>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select name="status" class="form-select form-select-sm">
                                    ...
                                    <option value="Pending"
                                        {{ old('status', $job?->status) === 'Pending' ? 'selected' : '' }}>
                                        Pending</option>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <select id="partyNameSelect" class="form-select form-select-sm w-100">
                                    @if ($job?->party_name)
                                        <option value="{{ $job->customer_id ?? $job->party_name }}" selected>
                                            {{ $job->party_name }}</option>
                                    @endif
                                </select>
                                <small class="text-danger d-none" id="party_name_err">The party name field is
                                    required.</small>
                            </div>
                            <div class="col-md-8">
                            ...
                            <div class="col-md-4">
                                ...
                                <select id="goodsNameSelect" class="form-select form-select-sm w-100">
                                    @if ($job?->goods_name)
                                        <option value="{{ $job->item_id ?? $job->goods_name }}" selected>
                                            {{ $job->goods_name }}</option>
                                    @endif
                                </select>
                                <small class="text-danger d-none" id="goods_name_err">The goods name field is
                                    required.</small>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Pack Quantity</label>
                                <input type="number" name="pack_quantity" class="form-control form-control-sm"
                                    step="0.001" value="{{ old('pack_quantity', $job?->pack_quantity) }}"
                                    placeholder="0.000">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Pack Unit</label>
                                <input type="text" name="pack_unit" class="form-control form-control-sm"
                                    value="{{ old('pack_unit', $job?->pack_unit) }}" placeholder="e.g. MT, PCS">
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Gross Weight</label>
                                <div class="input-group input-group-sm">
                                    <input type="number" name="gross_weight" class="form-control form-control-sm"
                                        step="0.001" value="{{ old('gross_weight', $job?->gross_weight) }}"
                                        placeholder="0.000">
                                    <input type="text" name="gross_weight_unit" class="form-control form-control-sm"
                                        style="max-width:100px;"
                                        value="{{ old('gross_weight_unit', $job?->gross_weight_unit ?? 'KGS') }}"
                                        placeholder="Unit">
                                </div>
                            </div>
                            <div class="col-md-4">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="number" name="net_weight" class="form-control form-control-sm"
                                        step="0.001" value="{{ old('net_weight', $job?->net_weight) }}"
                                        placeholder="0.000">
                                    <input type="text" name="net_weight_unit" class="form-control form-control-sm"
                                        style="max-width:100px;"
                                        value="{{ old('net_weight_unit', $job?->net_weight_unit ?? 'KGS') }}"
                                        placeholder="Unit">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="date" name="copy_doc_received_date" class="form-control form-control-sm"
                                    value="{{ old('copy_doc_received_date', $job?->copy_doc_received_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Original Doc. Received Date</label>
                                <input type="date" name="original_doc_received_date"
                                    class="form-control form-control-sm"
                                    value="{{ old('original_doc_received_date', $job?->original_doc_received_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">ETA Date</label>
                                <input type="date" name="eta_date" class="form-control form-control-sm"
                                    value="{{ old('eta_date', $job?->eta_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">HBL/HAWB No <span class="req">*</span></label>
                                <input type="text" name="hbi_hawb_no" class="form-control form-control-sm"
                                    value="{{ old('hbi_hawb_no', $job?->hbi_hawb_no) }}" placeholder="HBI/HAWB No"
                                    required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">HBL/HAWB Date</label>
                                <input type="date" name="hbi_hawb_date" class="form-control form-control-sm"
                                    value="{{ old('hbi_hawb_date', $job?->hbi_hawb_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">MBL/MAWB No</label>
                                <input type="text" name="mbl_mawb_no" class="form-control form-control-sm"
                                    value="{{ old('mbl_mawb_no', $job?->mbl_mawb_no) }}" placeholder="MBL/MAWB No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">MBL/MAWB Date</label>
                                <input type="date" name="mbl_mawb_date" class="form-control form-control-sm"
                                    value="{{ old('mbl_mawb_date', $job?->mbl_mawb_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Invoice No</label>
                                <input type="text" name="invoice_no" class="form-control form-control-sm"
                                    value="{{ old('invoice_no', $job?->invoice_no) }}" placeholder="Invoice No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Invoice Date</label>
                                <input type="date" name="invoice_date" class="form-control form-control-sm"
                                    value="{{ old('invoice_date', $job?->invoice_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">B/E No</label>
                                <input type="text" name="be_no" class="form-control form-control-sm"
                                    value="{{ old('be_no', $job?->be_no) }}" placeholder="B/E No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">B/E Date</label>
                                <input type="date" name="be_date" class="form-control form-control-sm"
                                    value="{{ old('be_date', $job?->be_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">LC No</label>
                                <input type="text" name="lc_no" class="form-control form-control-sm"
                                    value="{{ old('lc_no', $job?->lc_no) }}" placeholder="LC No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">LC Date</label>
                                <input type="date" name="lc_date" class="form-control form-control-sm"
                                    value="{{ old('lc_date', $job?->lc_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">LCA No</label>
                                <input type="text" name="lca_no" class="form-control form-control-sm"
                                    value="{{ old('lca_no', $job?->lca_no) }}" placeholder="LCA No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">LCA Date</label>
                                <input type="date" name="lca_date" class="form-control form-control-sm"
                                    value="{{ old('lca_date', $job?->lca_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">PO No</label>
                                <input type="text" name="po_no" class="form-control form-control-sm"
                                    value="{{ old('po_no', $job?->po_no) }}" placeholder="PO No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Mate Code</label>
                                <input type="text" name="mate_code" class="form-control form-control-sm"
                                    value="{{ old('mate_code', $job?->mate_code) }}" placeholder="Mate Code">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="lading_no" class="form-control form-control-sm"
                                    value="{{ old('lading_no', $job?->lading_no) }}" placeholder="Lading No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Lading Date</label>
                                <input type="date" name="lading_date" class="form-control form-control-sm"
                                    value="{{ old('lading_date', $job?->lading_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Flight No</label>
                                <input type="text" name="flight_no" class="form-control form-control-sm"
                                    value="{{ old('flight_no', $job?->flight_no) }}" placeholder="Flight No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Flight Date</label>
                                <input type="date" name="flight_date" class="form-control form-control-sm"
                                    value="{{ old('flight_date', $job?->flight_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Vessel's Name</label>
                                <input type="text" name="vessel_name" class="form-control form-control-sm"
                                    value="{{ old('vessel_name', $job?->vessel_name) }}" placeholder="Vessel's Name">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Boyge No</label>
                                <input type="text" name="boyge_no" class="form-control form-control-sm"
                                    value="{{ old('boyge_no', $job?->boyge_no) }}" placeholder="Boyge No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Vessel's ETB Agent</label>
                                <input type="text" name="vessel_etb_agent" class="form-control form-control-sm"
                                    value="{{ old('vessel_etb_agent', $job?->vessel_etb_agent) }}"
                                    placeholder="Vessel's ETB Agent">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">AL No</label>
                                <input type="text" name="al_no" class="form-control form-control-sm"
                                    value="{{ old('al_no', $job?->al_no) }}" placeholder="AL No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Sailed No</label>
                                <input type="text" name="sailed_no" class="form-control form-control-sm"
                                    value="{{ old('sailed_no', $job?->sailed_no) }}" placeholder="Sailed No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Arrived Date</label>
                                <input type="date" name="arrived_date" class="form-control form-control-sm"
                                    value="{{ old('arrived_date', $job?->arrived_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Common Lading Date</label>
                                <input type="date" name="common_lading_date" class="form-control form-control-sm"
                                    value="{{ old('common_lading_date', $job?->common_lading_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">W/Rent Due Date</label>
                                <input type="date" name="w_rent_due_date" class="form-control form-control-sm"
                                    value="{{ old('w_rent_due_date', $job?->w_rent_due_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Berthing</label>
                                <input type="text" name="berthing" class="form-control form-control-sm"
                                    value="{{ old('berthing', $job?->berthing) }}" placeholder="Berthing">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Berthing Date</label>
                                <input type="date" name="berthing_date" class="form-control form-control-sm"
                                    value="{{ old('berthing_date', $job?->berthing_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Shed No</label>
                                <input type="text" name="shed_no" class="form-control form-control-sm"
                                    value="{{ old('shed_no', $job?->shed_no) }}" placeholder="Shed No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Yard No</label>
                                <input type="text" name="yard_no" class="form-control form-control-sm"
                                    value="{{ old('yard_no', $job?->yard_no) }}" placeholder="Yard No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Transport Name</label>
                                <input type="text" name="transport_name" class="form-control form-control-sm"
                                    value="{{ old('transport_name', $job?->transport_name) }}"
                                    placeholder="Transport Name">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Transport No</label>
                                <input type="text" name="transport_no" class="form-control form-control-sm"
                                    value="{{ old('transport_no', $job?->transport_no) }}" placeholder="Transport No">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="rot_no" class="form-control form-control-sm"
                                    value="{{ old('rot_no', $job?->rot_no) }}" placeholder="Rot No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">B/L Weight Measurement</label>
                                <input type="text" name="bl_weight_measurement" class="form-control form-control-sm"
                                    value="{{ old('bl_weight_measurement', $job?->bl_weight_measurement) }}"
                                    placeholder="B/L Weight Measurement">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Jetty Sarker Name</label>
                                <input type="text" name="jetty_sarker_name" class="form-control form-control-sm"
                                    value="{{ old('jetty_sarker_name', $job?->jetty_sarker_name) }}"
                                    placeholder="Jetty Sarker Name">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Contact No</label>
                                <input type="text" name="contact_no" class="form-control form-control-sm"
                                    value="{{ old('contact_no', $job?->contact_no) }}" placeholder="Contact No">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Unit No</label>
                                <input type="text" name="unit_no" class="form-control form-control-sm"
                                    value="{{ old('unit_no', $job?->unit_no) }}" placeholder="Unit No">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="date" name="etb_date" class="form-control form-control-sm"
                                    value="{{ old('etb_date', $job?->etb_date?->format('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Delivery Date</label>
                                <input type="date" name="delivery_date" class="form-control form-control-sm"
                                    value="{{ old('delivery_date', $job?->delivery_date?->format('Y-m-d')) }}">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="consignee_name" class="form-control form-control-sm"
                                    value="{{ old('consignee_name', $job?->consignee_name) }}"
                                    placeholder="Name of consignee">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Consignee Address</label>
                                <input type="text" name="consignee_address" class="form-control form-control-sm"
                                    value="{{ old('consignee_address', $job?->consignee_address) }}"
                                    placeholder="Consignee address">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Agent Name</label>
                                <input type="text" name="agent_name" class="form-control form-control-sm"
                                    value="{{ old('agent_name', $job?->agent_name) }}" placeholder="Name of agent">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Agent Address</label>
                                <input type="text" name="agent_address" class="form-control form-control-sm"
                                    value="{{ old('agent_address', $job?->agent_address) }}" placeholder="Agent address">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="job-card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="commodity" class="form-control form-control-sm"
                                    value="{{ old('commodity', $job?->commodity) }}" placeholder="Commodity">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">No of Container</label>
                                <input type="text" name="no_of_container" class="form-control form-control-sm"
                                    value="{{ old('no_of_container', $job?->no_of_container) }}" placeholder="0">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">POL</label>
                                <input type="text" name="pol" class="form-control form-control-sm"
                                    value="{{ old('pol', $job?->pol) }}" placeholder="POL">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Destination</label>
                                <input type="text" name="destination" class="form-control form-control-sm"
                                    value="{{ old('destination', $job?->destination) }}" placeholder="Destination">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">ETD Date</label>
                                <input type="date" name="etd_date" class="form-control form-control-sm"
                                    value="{{ old('etd_date', $job?->etd_date?->format('Y-m-d')) }}">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4"
                style="position:sticky;top:70px;align-self:start;max-height:calc(100vh - 80px);display:flex;flex-direction:column;">
                                ...
                                <div class="col-6">
                                    ...
                                    <select name="currency_type" id="currencyType" class="form-select form-select-sm">
                                        ...
                                        @foreach ($currencies as $cur)
                                            <option value="{{ $cur }}"
                                                {{ old('currency_type', $job?->currency_type) === $cur ? 'selected' : '' }}>
                                                {{ $cur }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-6">
                                    <label class="form-label">Exchange Rate <small
                                            class="text-muted fw-normal">(BDT)</small></label>
                                    <input type="number" name="currency_rate" id="currencyRate"
                                        class="form-control form-control-sm text-end" step="0.000000001"
                                        value="{{ old('currency_rate', $job?->currency_rate) }}" placeholder="0.00">
                                </div>
                                <div class="col-6">
                                    <label class="form-label">Invoice Value <small id="invoiceCurLabel"
                                            class="text-muted fw-normal">(Orig. Cur.)</small></label>
                                    <input type="number" name="invoice_value_1"
                                        class="form-control form-control-sm charge-c1 text-end" step="0.01"
                                        value="{{ old('invoice_value_1', $job?->invoice_value_1) }}"
                                        data-key="invoice_value" placeholder="0.00">
                                </div>
                                <div class="col-6">
                                    <label class="form-label">Invoice Value <small
                                            class="text-muted fw-normal">(BDT)</small></label>
                                    <input type="number" name="invoice_value_2" id="c2_invoice_value"
                                        class="form-control form-control-sm ro-field text-end" readonly step="0.000000001"
                                        value="{{ old('invoice_value_2', $job?->invoice_value_2) }}" placeholder="0.00">
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Assessable Value <span class="req">*</span></label>
                                    <input type="number" name="assessable_value" id="assessableValue"
                                        class="form-control form-control-sm text-end" step="0.01"
                                        value="{{ old('assessable_value', $job?->assessable_value) }}"
                                        placeholder="0.00">
                                </div>
                            </div>
                        </div>
                            ...
                            <div>
                                <table class="charge-tbl">
                                    <thead>
                                        <tr>
                                            <th>Description</th>
                                            <th class="text-end" style="width:120px;">Amount <small
                                                    class="text-muted fw-normal">(BDT)</small></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @php
                                        ...
                                        @foreach ($taxRows as [$label, $key])
                                            <tr>
                                                <td>{{ $label }}</td>
                                                <td><input type="number" name="{{ $key }}_amount"
                                                        id="tax_{{ $key }}"
                                                        class="form-control form-control-sm tax-amount text-end"
                                                        step="0.000000001"
                                                        value="{{ old($key . '_amount', $job?->{$key . '_amount'}) }}"
                                                        placeholder="0.00"></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                ...
                                <div class="totals-panel">
                                    <table class="charge-tbl">
                                        <tr>
                                            ...
                                            <td style="width:120px;"><input type="number" id="totalDutyAmount"
                                                    class="form-control form-control-sm ro-field text-end fw-semibold"
                                                    readonly placeholder="0.00"></td>
                                            <td style="width:120px;"><input type="number" id="totalDutyAmountBdt"
                                                    class="form-control form-control-sm ro-field text-end fw-semibold"
                                                    readonly placeholder="0.00"></td>
                                        </tr>
                                    </table>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body py-3">
                    <div class="mb-2">
                        ...
                        <input type="text" id="qi_name" class="form-control form-control-sm"
                            placeholder="e.g. Rice (IRRI)">
...
@push('scripts')
    <script>
        ...
        $(function() {
            ...
            $('select[name="job_type_id"], select[name="port_id"], select[name="country_of_origin"], select[name="status"]')
                .each(function() {
                    var $el = $(this);
                    $el.select2({
                        theme: 'bootstrap-5',
            ...
            $('#btnSaveQuickItem').on('click', function() {
                ...
                if (!name) {
                    $('#qi_name').addClass('is-invalid');
                    $('#qi_name_err').text('Required.');
                    ok = false;
                } else {
                ...
                $('#btnSaveQuickItem').prop('disabled', true).html(
                    '<span class="spinner-border spinner-border-sm me-1"></span>Saving...');

                $.ajax({
                        url: QUICK_ITEM_STORE,
                        method: 'POST',
                        data: {
                    ...
                    .fail(function(xhr) {
                        ...
                        if (errs) {
                            if (errs.item_name) {
                                $('#qi_name').addClass('is-invalid');
                                $('#qi_name_err').text(errs.item_name[0]);
                            }
                            if (errs.purchase_unit) {
                                $('#qi_unit').addClass('is-invalid');
                                $('#qi_unit_err').text(errs.purchase_unit[0]);
                            }
                            if (errs.item_price) {
                                $('#qi_price').addClass('is-invalid');
                                $('#qi_price_err').text(errs.item_price[0]);
                            }
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: xhr.responseJSON?.message || 'Save failed.'
                            });
                        }
                    })
                    .always(function() {
                        $('#btnSaveQuickItem').prop('disabled', false).html(
                            '<i class="fa fa-save me-1"></i> Save & Select');
                    });
            });
            ...
            $('form').on('submit', function(e) {
                ...
                if (!$('select[name="job_type_id"]').val()) {
                    $('select[name="job_type_id"]').next('.select2-container').find('.select2-selection')
                ...
                if (!$('select[name="port_id"]').val()) {
                    $('select[name="port_id"]').next('.select2-container').find('.select2-selection')
                        .addClass('border-danger');
                ...
                if (!$('#partyNameHidden').val()) {
                    $('#partyNameSelect').next('.select2-container').find('.select2-selection').addClass(
                        'border-danger');
                ...
                if (!$('#goodsNameHidden').val()) {
                    $('#goodsNameSelect').next('.select2-container').find('.select2-selection').addClass(
                        'border-danger');
