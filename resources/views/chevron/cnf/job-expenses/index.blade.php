...
@push('styles')
    <style>
        ...
        #expTable thead tr:last-child th {
            background: #f8f9fa;
        }

        #expTable thead tr:last-child th input.form-control {
            min-width: 72px;
            width: 100%;
        ...
        .exp-table-wrapper::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .exp-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .exp-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #expTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
...
@section('content')
    <div class="page-header">
        ...
        @if (auth()->user()->hasPermission('cnf.job-expense.create'))
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><button class="dropdown-item"
                            onclick="$('#expTable').DataTable().button('.buttons-csv').trigger()"><i
                                class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#expTable').DataTable().button('.buttons-excel').trigger()"><i
                                class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#expTable').DataTable().button('.buttons-pdf').trigger()"><i
                                class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                    <li>
                        <hr class="dropdown-divider">
                    </li>
                    <li><button class="dropdown-item"
                            onclick="$('#expTable').DataTable().button('.buttons-print').trigger()"><i
        ...
        $(function() {
            var table = $('#expTable').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'total_expense_amount_fmt',
                        name: 'total_expense_amount',
                        className: 'text-end'
                    },
                    {
                        data: 'total_approved_amount_fmt',
