...
@push('styles')
    <style>
        ...
        #portsTable thead tr:last-child th {
            background: #f8f9fa;
        }

        #portsTable thead tr:last-child th input.form-control {
            min-width: 72px;
            width: 100%;
        ...
        .ports-table-wrapper::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .ports-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .ports-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #portsTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><button class="dropdown-item"
                            onclick="$('#portsTable').DataTable().button('.buttons-csv').trigger()"><i
                                class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#portsTable').DataTable().button('.buttons-excel').trigger()"><i
                                class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#portsTable').DataTable().button('.buttons-pdf').trigger()"><i
                                class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                    <li>
                        <hr class="dropdown-divider">
                    </li>
                    <li><button class="dropdown-item"
                            onclick="$('#portsTable').DataTable().button('.buttons-print').trigger()"><i
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" id="portName" class="form-control" placeholder="e.g. Chittagong Port"
                                    required>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Port Code <span class="text-danger">*</span></label>
                                <input type="text" id="portCode" class="form-control" placeholder="e.g. CGP"
                                    maxlength="10" style="text-transform:uppercase">
                            </div>
                            <div class="col-12">
                                <label class="form-label">Prefix</label>
                                <input type="text" id="portPrefix" class="form-control" placeholder="e.g. CGP-"
                                    maxlength="20">
                            </div>
                            <div class="col-12">
    ...
    <script>
        ...
        $(function() {
            table = $('#portsTable').DataTable({
                ...
                language: {
                    emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No ports yet.</div>'
                },
            });
            ...
            $('#portForm').on('submit', function(e) {
                ...
                if (!$('#portName').val().trim()) {
                    $('#portName').addClass('is-invalid').after(
                        '<div class="invalid-feedback">Name is required.</div>');
                    return;
                }
                if (!$('#portCode').val().trim()) {
                    $('#portCode').addClass('is-invalid').after(
