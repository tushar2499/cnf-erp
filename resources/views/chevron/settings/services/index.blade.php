...
@push('styles')
    <style>
        ...
        #servicesTable thead tr:last-child th {
            background: #f8f9fa;
        }

        #servicesTable thead tr:last-child th input.form-control {
            min-width: 72px;
            width: 100%;
        ...
        .services-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .services-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #servicesTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><button class="dropdown-item"
                            onclick="$('#servicesTable').DataTable().button('.buttons-csv').trigger()"><i
                                class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#servicesTable').DataTable().button('.buttons-excel').trigger()"><i
                                class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#servicesTable').DataTable().button('.buttons-pdf').trigger()"><i
                                class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                    <li>
                        <hr class="dropdown-divider">
                    </li>
                    <li><button class="dropdown-item"
                            onclick="$('#servicesTable').DataTable().button('.buttons-print').trigger()"><i
                ...
                <form id="serviceForm">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" id="svcName" class="form-control"
                                placeholder="e.g. Sea Import Clearance" required>
                        </div>
    ...
    <script>
        ...
        $(function() {
            table = $('#servicesTable').DataTable({
                ...
                language: {
                    emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No services yet.</div>'
                },
            });
                    ...
                    .fail(function(xhr) {
                        if (xhr.status === 422) {
                            $.each(xhr.responseJSON.errors, function(field, msgs) {
                                if (field === 'name') {
                                    $('#svcName').addClass('is-invalid').after(
