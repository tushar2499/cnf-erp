                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" id="headName" class="form-control"
                                    placeholder="e.g. Port Handling Fee">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Type <span class="text-danger">*</span></label>
                                <select id="headType" class="form-select select2">
                ...
                <div class="modal-body">
                    ...
                    <div id="importStep1">
                        <div class="alert alert-info d-flex gap-2 align-items-start" style="font-size:12px">
                            ...
                            <div>
                                ...
                                <strong>Name</strong>, <strong>Category</strong>, <strong>Type</strong> (Internal/External),
                                <strong>Amount</strong>, <strong>Status</strong> (Active/Inactive).<br>
                    ...
                    <div id="importStep2" style="display:none">
                        ...
                        <div class="mt-2 text-muted" style="font-size:11px">
                            ...
                            Rows with <span class="badge bg-danger" style="font-size:10px">Warning</span> have unmatched
                            category or invalid type — they will still import with null category / default type.
                        </div>
                    </div>
    ...
    <script>
        ...
        $(function() {
            ...
            table = $('#expenseHeadsTable').DataTable({
                ...
                language: {
                    emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No expense heads yet.</div>'
                },
            });
            ...
            $('#expenseHeadForm').on('submit', function(e) {
                ...
                let valid = true;
                if (!$('#headName').val().trim()) {
                    showFieldError($('#headName'), 'Name is required.');
                    valid = false;
                }
                if (!$('#headCategory').val()) {
                    showFieldError($('#headCategory'), 'Category is required.');
                    valid = false;
                }
                if (!$('#headType').val()) {
                    showFieldError($('#headType'), 'Type is required.');
                    valid = false;
                }
                if (!valid) return;
                ...
                $.ajax({
                    ...
                    .fail(function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            if (errors.name) {
                                showFieldError($('#headName'), errors.name[0]);
                            }
                            if (errors.expense_category_id) {
                                showFieldError($('#headCategory'), errors.expense_category_id[0]);
                            }
                            if (errors.type) {
                                showFieldError($('#headType'), errors.type[0]);
                            }
                            if (errors.amount) {
                                showFieldError($('#headAmount'), errors.amount[0]);
                            }
                        } else {
                            Swal.fire({
                                icon: 'error',
            ...
            $('#btnPreview').on('click', function() {
                ...
                $('#btnPreview').prop('disabled', true).html(
                ...
                $.ajax({
                    ...
                    .fail(function(xhr) {
                        Swal.fire({
                            icon: 'error',
                            title: xhr.responseJSON?.message || 'Failed to parse file.'
                        });
                    })
                    .always(function() {
                        $('#btnPreview').prop('disabled', false).html(
                            '<i class="fa fa-eye me-1"></i> Preview');
                    });
            });
            ...
            function renderPreview(rows) {
                ...
                if (rows.length === 0) {
                    html =
                        '<tr><td colspan="7" class="text-center text-muted py-3">No valid rows found in file.</td></tr>';
                }
            ...
            $('#btnConfirmImport').on('click', function() {
                ...
                $.ajax({
                    ...
                    .fail(function(xhr) {
                        Swal.fire({
                            icon: 'error',
                            title: xhr.responseJSON?.message || 'Import failed.'
                        });
                    })
                    .always(function() {
