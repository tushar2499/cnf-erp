...
@push('styles')
    <style>
        ...
        #jobTypesTable thead tr:last-child th {
            background: #f8f9fa;
        }

        #jobTypesTable thead tr:last-child th input.form-control {
            min-width: 72px;
            width: 100%;
            box-sizing: border-box;
        }

        .job-types-table-wrapper {
            max-height: 65vh;
            overflow: auto;
        }

        .job-types-table-wrapper::-webkit-scrollbar {
            width: 6px;
            height: 6px;
        }

        .job-types-table-wrapper::-webkit-scrollbar-track {
            background: #f1f1f1;
        }

        .job-types-table-wrapper::-webkit-scrollbar-thumb {
            background: #c1c1c1;
            border-radius: 3px;
        }

        #jobTypesTable_wrapper>.row:last-child {
            position: sticky;
            bottom: 0;
            background: #fff;
            z-index: 3;
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                    <li><button class="dropdown-item"
                            onclick="$('#jobTypesTable').DataTable().button('.buttons-csv').trigger()"><i
                                class="fa fa-file-csv me-2 text-secondary"></i>CSV</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#jobTypesTable').DataTable().button('.buttons-excel').trigger()"><i
                                class="fa fa-file-excel me-2 text-success"></i>Excel</button></li>
                    <li><button class="dropdown-item"
                            onclick="$('#jobTypesTable').DataTable().button('.buttons-pdf').trigger()"><i
                                class="fa fa-file-pdf me-2 text-danger"></i>PDF</button></li>
                    <li>
                        <hr class="dropdown-divider">
                    </li>
                    <li><button class="dropdown-item"
                            onclick="$('#jobTypesTable').DataTable().button('.buttons-print').trigger()"><i
                    ...
                    <div class="modal-body">
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" id="jtCode" class="form-control" placeholder="e.g. SI"
    ...
    <script>
        ...
        $(function() {
            table = $('#jobTypesTable').DataTable({
                ...
                language: {
                    emptyTable: '<div class="text-center py-3 text-muted"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No job types yet.</div>'
                },
            });
            ...
            $('#jobTypeForm').on('submit', function(e) {
                ...
                if (!$('#jtName').val().trim()) {
                    $('#jtName').addClass('is-invalid').after(
                        '<div class="invalid-feedback">Name is required.</div>');
                    return;
                }
                if (!$('#jtCode').val().trim()) {
                    $('#jtCode').addClass('is-invalid').after(
